#!/usr/bin/env python3
"""
Enhanced Academic Report Generator for SVM Models

This module generates comprehensive academic reports for SVM training experiments,
...
importance analysis for Support Vector Machines.
"""
...
class AcademicReportGenerator:
    """Enhanced academic report generator for SVM models with detailed analysis"""
    
    def __init__(self):
        self.report_template = """# SVM Training Report
...
This report documents a comprehensive SVM training experiment conducted for academic research and reproducibility purposes. The experiment involved{hyperopt_text} cross-validated model training with detailed performance analysis and data validation.
...
### {arch_section_number}.1 SVM Configuration
...
"""
    ...
    def generate_report(self, 
                       ...
                       data_validation_results: Optional[Dict[str, Any]] = None) -> str:
    ...
    def _get_hyperparameter_ranges(self) -> Dict[str, Any]:
        """Get the hyperparameter search ranges used in SVM optimization"""
        return {
            "C": {
                "range": [1e-3, 1e3],
                "scale": "log",
                "description": "Regularization parameter. Controls the trade-off between maximizing the margin and minimizing the classification error."
            },
            "kernel": {
                "options": ["linear", "rbf", "poly", "sigmoid"],
                "description": "Kernel type to be used in the algorithm. Maps inputs to higher-dimensional spaces."
            },
            "degree": {
                "range": [2, 5],
                "step": 1,
                "description": "Degree of the polynomial kernel function ('poly'). Ignored by other kernels."
            },
            "gamma": {
                "options": ["scale", "auto"],
                "alt_range": [1e-5, 1e1],
                "scale": "log",
                "description": "Kernel coefficient for 'rbf', 'poly' and 'sigmoid'. Defines the influence of training samples."
            },
            "coef0": {
                "range": [0.0, 10.0],
            ...
            "class_weight": {
                ...
                "description": "Weights for classes to address imbalanced datasets. Only for classification tasks."
            },
            "epsilon": {
                "range": [1e-4, 1.0],
                "scale": "log",
                "description": "Specifies the epsilon-tube within which no penalty is associated in the loss function. Only for regression tasks."
            }
        }
    ...
    def _collect_report_data(self, 
                           ...
                           training_results: Dict[str, Any],
                           hyperopt_results: Optional[Dict[str, Any]] = None,
                           data_validation_results: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """Collect report data including validation results"""
    ...
    def _format_dataset_info(self, metadata: Dict[str, Any]) -> str:
        ...
        lines = []
        print("--------------------------------")
        print(metadata["tool_input_details"])
        print(metadata["tool_input_details"]["data_source"])
    ...
    def _format_preprocessing_section(self, metadata: Dict[str, Any]) -> str:
        ...
        if not preprocessing_applied:
            return """No feature scaling was applied to the input data. While SVMs can process raw numerical features, they are highly sensitive to feature scales, especially when using non-linear kernels like RBF. Standardization or normalization is generally recommended for SVMs.
...
- **Algorithm Performance**: Without scaling, features with larger numeric ranges may dominate the model, potentially degrading performance especially with distance-based kernels.
- **Future Improvement**: Consider applying StandardScaler (z-score normalization) for potentially better results, especially if using RBF kernel.
"""
        else:
            return """The data underwent comprehensive preprocessing to optimize SVM model performance, enhance training stability, and improve generalization.
...
"""
    ...
    def _format_hyperopt_section(self, hyperopt_results: Dict[str, Any]) -> str:
        ...
        if best_params:
            ...
            c_value = best_params.get('C', 1.0)
            gamma = best_params.get('gamma', 'scale')
            
            lines.extend([
                f"- **Kernel Selection**: {kernel} kernel was found optimal for this dataset",
                f"- **Regularization**: C={c_value} provides the optimal balance between margin size and classification error",
            ])
            
            if kernel == 'poly':
                lines.append(f"- **Polynomial Degree**: {best_params.get('degree', 3)} provides best fit")
                lines.append(f"- **Independent Term**: coef0={best_params.get('coef0', 0.0)} optimizes the decision boundary")
            elif kernel == 'rbf':
                lines.append(f"- **Gamma Parameter**: {gamma} defines optimal influence radius of training samples")
            
            if 'shrinking' in best_params:
                lines.append(f"- **Shrinking Heuristic**: {'Enabled' if best_params.get('shrinking') else 'Disabled'} for optimization efficiency")
                
            if 'max_iter' in best_params:
                lines.append(f"- **Iterations**: {best_params.get('max_iter')} maximum iterations provides sufficient convergence")
                
            if 'class_weight' in best_params:
                weight = best_params.get('class_weight')
                if weight:
                    lines.append(f"- **Class Weighting**: {weight} addresses class imbalance in the dataset")
    ...
    def _format_visualization_section(self, files: Dict[str, List[str]], metadata: Dict[str, Any] = None) -> str:
        ...
        # 检查决策边界图 - SVM特有
        has_decision_boundary = False
        decision_boundary_path = ""
        if 'decision_boundary.png' in files.get('plots', []) or 'decision_boundary.png' in files.get('cv_data', []):
            has_decision_boundary = True
            if 'decision_boundary.png' in files.get('cv_data', []):
                decision_boundary_path = "cross_validation_data/decision_boundary.png"
            else:
                decision_boundary_path = "decision_boundary.png"
...
"""
        ...
        # 添加决策边界图（SVM特有）
        if has_decision_boundary:
            section += f"""#### SVM Decision Boundary Visualization

The SVM decision boundary visualization shows how the model separates classes in the feature space. For higher-dimensional data, this represents a 2D projection of the boundary.

<div style="text-align: center; margin: 20px 0;">
    <img src="{decision_boundary_path}" alt="SVM Decision Boundary" style="max-width: 100%; height: auto; border-radius: 8px; box-shadow: 0 4px 8px rgba(0,0,0,0.1);">
    <p style="font-style: italic; color: #666; margin-top: 10px;">SVM Decision Boundary with Support Vectors Highlighted</p>
</div>
...
"""
        elif task_type == "classification" and not has_roc_curves and not has_decision_boundary:
            section += """#### Classification Performance Analysis

<div class="info-box" style="background-color: #e7f3ff; border-left: 4px solid #2196F3; padding: 15px; margin: 15px 0; border-radius: 4px;">
    <strong>📊 Note:</strong> Cross-validation ROC curve and decision boundary visualizations would be displayed here when available.
</div>
...
"""
    ...
    def _format_model_config(self, metadata: Dict[str, Any]) -> str:
        ...
        # 显式处理几个关键参数
        key_params = ['C', 'kernel', 'degree', 'gamma', 'coef0']
    ...
    def _format_key_findings(self, results: Dict[str, Any], metadata: Dict[str, Any], has_hyperopt: bool) -> str:
        ...
        if 'cv_scores' in results:
            ...
            if isinstance(cv_scores, dict) and 'test_score' in cv_scores:
                scores = cv_scores['test_score']
                mean_score = np.mean(scores)
                findings.append(f"1. **Model Performance**: The SVM achieved a cross-validation score of {mean_score:.6f}")
    ...
    def _format_technical_implementation(self) -> str:
        ...
        return """- **Framework**: scikit-learn SVM implementation for Support Vector Machines.
...
| **Model ID** | `{model_id}` |"""
        ...
        md_content += f"""
| **Framework** | SVM (scikit-learn interface) |
...
|------|-------------|"""
        ...
        if results.get('confidence_scores'):
            ...
            if task_type == 'classification':
                ...
                md_content += "- **Primary Method**: Distance from decision boundary in SVM\n"
                md_content += "- **Formula**: `confidence = 1 / (1 + exp(-abs(distance_to_margin)))`\n"
                ...
                md_content += "- **Fallback Method**: For prediction scenarios without margin information, uses inverse relationship with prediction magnitude\n\n"
                
                md_content += "**Technical Details:**\n"
                md_content += "- Each prediction's confidence is based on how far it is from the SVM decision boundary (margin)\n"
                md_content += "- Predictions far from the boundary (well within their class region) have high confidence\n"
                md_content += "- Predictions close to the boundary have low confidence\n"
                md_content += "- Support vectors (training points exactly on the margin) would have medium confidence\n\n"
        ...
        md_content += f"*MCP SVM Tool - Prediction Experiment Report*\n"
    ...
    def _is_direct_feature_importance_format(self, data: Dict[str, Any]) -> bool:
        ...
        for key, value in data.items():
            if isinstance(value, dict):
                # Check if it has expected feature importance metrics - now SVM specific
                expected_keys = {'weight', 'coefficient', 'permutation_mean', 'permutation_std', 'rfe_rank'}
